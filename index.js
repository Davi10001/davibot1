...
const config = {
    A187: '📍 𝐃𝐀𝐕𝐈 𝐁𝐎𝐓📍',
    instagram: '@davi_mendes_11',
    nomer: 'wa.me/553188514445',
    aktif: '24 JAM',
    youtube: 'TENHO NAO MAS SE INSCREVE LA NO CANAL DO CRXX TV',
    whatsapp: 'http://chat.whatsapp.com/BMrs4VGgqSsEHaizmgQUTn',
       ...
       case 'lirik':
           lirik(value)
               .then(data => {
                   ...
                   let hasil = `📍𝑳𝑬𝑻𝑹𝑨 𝑫𝑨 𝑴𝑼𝑺𝑰𝑪𝑨📍 *${value}* \n\n\n${lirik}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'fb':
           fb(value)
               .then(data => {
                   ...
                   let hasil = ` 𝑬𝑺𝑪𝑶𝑳𝑯𝑬 𝑨 𝑹𝑬𝑺𝑶𝑳𝑼𝑪𝑨𝑶 𝑴𝑨𝑵 \n\n\n HD ${resultHD} \n\n\n SD ${resultSD}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
...
case 'puisi2':
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = ` 𝑨𝑸 𝑻𝑨 𝑶𝑺 𝑨𝑪𝑶𝑹𝑫𝑬𝑺 𝑫𝑨 𝑴𝑼𝑺𝑰𝑪𝑨 𝑺𝑬𝑼 𝑻𝑶𝑪𝑨𝑫𝑶𝑹 𝑫𝑬 𝑽𝑰𝑶𝑳𝑨𝑶 𝑲𝑲𝑲𝑲𝑲𝑲 *${value}* querida ♥️\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'wikien':
           wikien(value)
               .then(data => {
                   const { result } = data
                   let hasil = `*𝑷𝑬𝑳𝑶 𝑸 𝑬𝑼 𝑽𝑰 𝑨𝑸 𝑵𝑨 𝑾𝑰𝑲𝑰𝑷𝑬𝑫𝑰𝑨:* \n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
